...
(function() {
	...
	streamIO.readStream(filename, options)(function(err, res) {
		counter++;
		outStream.pushNext(err, res);
	}, function(err) {
		if (err) {
			log(err);
		}
		log("stream completed, pushes " + counter);
		outStream.complete(err);
	})

	// streamIO.readUrlStream(url, true)(function(err, res){counter++;
	// outStream.pushNext(err,res); }, function(err) { if(err) {log(err);}
	// log("stream completed, pushes " + counter); outStream.complete(err);});
